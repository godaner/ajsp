...
func fetch(hc *http.Client) (rs []interface{}) {
	defer func() {
		if err := recover(); err != nil {
			log.Println("djd fetch : PANIC PANIC PANIC PANIC PANIC PANIC PANIC PANIC PANIC PANIC PANIC , err is ", err)
		}
	}()
	req, err := http.NewRequest("POST", flag.HttpAddr+"/api/approval/wf/task/upcoming", bytes.NewBufferString(`{"page":1,"rows":`+fmt.Sprint(flag.N)+`,"orderBy":{"updateTime":"desc"},"affairName":null,"likeMap":{},"between":{},"in":{"bizStatus":["41","32","34"]}}`))
	if err != nil {
		panic(err)
	}
...
func do(hc *http.Client, r interface{}) {
	...
	if err != nil {
		log.Printf("djd do record err 1 , err is : %v!", err)
		return
	}
	req.Header.Set("Authorization", flag.Auth)
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := hc.Do(req)
	if err != nil {
		log.Printf("djd do record err 2 , err is : %v!", err)
		return
	}
	body, err := ioutil.ReadAll(resp.Body)
	if body != nil {
	...
	if err != nil {
		log.Printf("djd do record err 3 , err is : %v!", err)
		return
	}
}
