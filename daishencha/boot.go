...
func do() {
	...
	req, err := http.NewRequest("POST", httpAddr+"/api/approval/wf/task/upcoming", bytes.NewBufferString(`{"page":1,"rows":`+fmt.Sprint(n)+`,"orderBy":{"updateTime":"desc"},"affairName":null,"likeMap":{},"between":{},"in":{"bizStatus":["21","22","23","24","25","26","27,","28","31","33"]}}`))
	if err != nil {
		panic(err)
	}
	...
	for index, r := range rs {
		...
		req, err := http.NewRequest("POST", httpAddr+"/api/approval/dth/affair/submitAffair", bytes.NewBufferString(`{"affairId":"`+affairId+`","shardKey":"`+shardKey+`","handleType":6,"auditAdvice":"1","auditAdviceInfo":"同意","base64Json":"","attIds":""}`))
		if err != nil {
